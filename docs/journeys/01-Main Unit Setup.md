# Main Control Unit Setup User Journey

## Goal

Using a connected touch screen and, optionally, a keyboard, ensure that the Main Control Unit can receive, store, and retrieve all basic setup information needed to be functional.
...
- The Main Control Unit has a connected touch screen so the user can interact with it directly.
- A keyboard may be connected, but the journey should remain usable from the touch screen.
- The setup UI is available locally on the Main Control Unit, even before the unit has joined a network.
...
- The system can determine whether the Main Control Unit is currently connected to a network.
...
**Then** Main Control Unit Setup is required, and the first step is General Information.

## Journey

**Given** the **Main Control Unit** starts up.
**When** the main configuration is not found in the database.
**Then** Main Control Unit Setup is required, and the setup user journey begins.

**Given** the Main Control Unit enters the setup user journey.
**When** the setup user journey begins and the Main Control Unit is not connected to any form of network.
...
**Given** the Main Control Unit is connected to a network. 
...
- The Main Control Unit can connect to the selected network, unless already connected.
