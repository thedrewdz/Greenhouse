...
When the Main Control Unit already has general configuration but starts without a network connection, alert the user and allow them to connect to a different network from the unit.
...
- The setup UI or dashboard is available locally on the Main Control Unit, even while the unit is offline.
...
**Given** the Main Control Unit has general configuration.
...
- A configured Main Control Unit can start while offline.
